...
simulation.model.hatch.LS.APIS <- do.call(rbind, lapply(unique(simulation.data.filt$year.class), function(i) {
  print(i)
...
simulation.anomaly.LS.APIS <- simulation.model.hatch.LS.APIS %>%
  ...
  mutate(mean.hist.spawn.yday = simulation.model.hist.mean.LS.APIS$mean.hist.spawn.yday,
         ...
         mean.hist.hatch.length = simulation.model.hist.mean.LS.APIS$mean.hist.hatch.length,
