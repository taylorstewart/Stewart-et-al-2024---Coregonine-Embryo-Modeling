...
## Stewart et al. 2021
model.data.ST <- read_excel("/Users/taylor/SynologyDrive/Cisco-Climate-Change/Coregonine-Temp-Embryo-EuropeanWhitefish/data/Coregonine-Temperature-Experiment-EuropeFrance-Hatch.xlsx", sheet = "hatching") %>% 
...
## Fit Semilog Model
model.ST <- lm(log.dpf.recip ~ temp.c, data = model.data.ST)

## Take antilog from daily semilog output, accumulate across days
model.ST.perc <- temp.all %>%
  group_by(year) %>% 
  filter(date >= as.Date(mu.spawn, origin = paste0(year, "-01-01"))) %>% 
  mutate(perc.day = (10^(coef(model.ST)[[1]] + coef(model.ST)[[2]] * temp.c))*100,
         perc.cum = cumsum(perc.day),) %>% 
  filter(perc.cum <= 100) %>%
  mutate(ADD = cumsum(temp.c))
...
model.hatching.all <- bind_rows(model.EC.perc.max, model.ST.perc.max) %>% 
  mutate(model = factor(model, ordered = TRUE, levels = c("ST", "EC")),
         yday = yday(date))
