...
simulation.files <- grep(pattern = "(?=.*simstrat)(?=.*watertemp)(?=.*annecy)",
...
simulation.data.comb <- simulation.data.filt %>% 
  ...
  summarize(mean.temp_c = mean(temp_c)) %>% ungroup()


#### RUN MODEL SIMULATIONS -----------------------------------------------------------------------

simulation.model.hatch.LA <- do.call(rbind, lapply(unique(simulation.data.comb$year.class), function(i) {
  print(i)
...
simulation.model.hist.mean.LA <- simulation.model.hatch.LA %>% 
...
## calculate anomaly
simulation.anomaly.LA <- simulation.model.hatch.LA %>%
  filter(surv == 1) %>% 
  group_by(scenario) %>% 
  distinct(spawn.date, .keep_all = TRUE) %>% 
  mutate(mean.hist.spawn.yday = simulation.model.hist.mean.LA$mean.hist.spawn.yday,
         mean.hist.hatch.yday = simulation.model.hist.mean.LA$mean.hist.hatch.yday,
         mean.hist.hatch.length = simulation.model.hist.mean.LA$mean.hist.hatch.length,
         mean.hist.dpf = simulation.model.hist.mean.LA$mean.hist.dpf) %>% 
...
simulation.anomaly.slope.LA <- do.call(rbind, lapply(trait.list, function(i) {
  tmp.rcp <- simulation.anomaly.LA %>%  select(year.class, scenario, all_of(i)) %>% 
...
write.csv(simulation.anomaly.slope.LA, "data/anomaly-slopes/lake-annecy-slope.csv", row.names = FALSE)


simulation.anomaly.comp.LA <- do.call(rbind, lapply(trait.list, function(i) {
  tmp.rcp.factor <- simulation.anomaly.LA %>%  select(year.class, scenario, i) %>% 
    filter(scenario != "Historical") %>% 
...
write.csv(simulation.anomaly.comp.LA, "data/anomaly-slopes/lake-annecy-multComp.csv", row.names = FALSE)

## Clean environment
rm("simulation.data", "model.parameters", "simulation.model.hist.mean.LA", "survival.reg", "trait.list", "simulation.data.comb", "simulation.data.filt", "model.locations", "simulation.files")
