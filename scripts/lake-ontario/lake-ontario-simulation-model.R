...
ggplot(simulation.model.hatch.spawnPlot, aes(x = year.class, y = spawn.yday.plot)) +
  ...
  scale_y_date(limits = c(as.Date("1970-11-05"), as.Date("1970-12-27")),
               date_breaks = "7 days", date_labels =  "%b %d", expand = c(0, 0)) + 
  #scale_y_continuous(limits = c(320, 380), breaks = seq(320, 380, 10), expand = c(0, 0)) +
  scale_fill_gradient2(limits = c(105, 170), breaks = seq(110, 170, 10),
                       low = "#ca0020", mid = "#ffffbf", high = "#0571b0", midpoint = 137.5) +
...
ggsave("figures/lake-ontario/lake-ontario-simulation-heatmap2.png", width = 18, height = 7, dpi = 300)
