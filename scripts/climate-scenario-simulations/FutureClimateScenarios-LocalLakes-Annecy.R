...
lapply(seq(2010, 2020, 10), function(j) {
  ...
  lapply(seq(0, 20, 1), function(k) {
    ...
    write.csv(tmp2, paste0("data/climate-simulations/simstrat-summaries/lake-", lake, "/", j, "-", toupper(lake), "-", k, "m.csv"), row.names = FALSE)
    ...
    ggsave(paste0("figures/climate-scenarios/simstrat-summaries/lake-", lake, "/", j, "-", toupper(lake), "-", k, "m.png"), dpi = 300, width = 8, height = 6)
  })
})
