...
plot.spawn.anomaly.LA <- ggplot(simulation.anomaly.LA, aes(x = year.class, y = mean.spawn.yday.anomaly, group = scenario)) +
  ...
  scale_color_manual(values = c("gray50","#2c7bb6", "#fdae61",  "#d7191c")) +
  scale_linetype_manual(values = c("solid", "longdash", "twodash", "solid")) +
  labs(y = "Spawning Date\nAnomaly (Days)", title = "Lake Annecy - Whitefish") +
  theme_few() + 
...
plot.all <- grid.arrange(
  ...
  arrangeGrob(
    arrangeGrob(plot.spawn.anomaly.LC + theme(legend.position = "none"),
                plot.dpf.anomaly.LC,
...
ggsave("figures/lavaretus-simulation-anomaly.png", plot = plot.all, width = 13, height = 12, dpi = 300)
