...
simulation.model.hatch.LK <- do.call(rbind, lapply(unique(simulation.data.filt$year.class), function(i) {
  print(i)
...
simulation.anomaly.LK <- simulation.model.hatch.LK %>%
  ...
  mutate(mean.hist.spawn.yday = simulation.model.hist.mean.LK$mean.hist.spawn.yday,
         ...
         mean.hist.hatch.length = simulation.model.hist.mean.LK$mean.hist.hatch.length,
...
write.csv(simulation.anomaly.slope.LK, "data/anomaly-slopes/lake-konnevesi-albula-slope.csv", row.names = FALSE)
